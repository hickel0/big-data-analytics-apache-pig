...
fs -rm -r -f /user/hadoop/outputs/clean
fs -rm -r -f /user/hadoop/outputs/samples
fs -rm -r -f /user/hadoop/outputs/team_summary

-- 1. LOAD RAW DATA FROM HDFS
team_raw = LOAD '/user/hadoop/data/team_traditional.csv'
    USING PigStorage(',')
    AS (
        gameid:chararray,
        date:chararray,
...
STORE team_clean INTO '/user/hadoop/outputs/clean' USING PigStorage('\t');
...
STORE team_summary INTO '/user/hadoop/outputs/team_summary' USING PigStorage('\t');

-- OPTIONAL: a small sample for debugging
-- sample = LIMIT team_clean 5;
-- STORE sample INTO '/user/hadoop/outputs/samples' USING PigStorage('\t');
